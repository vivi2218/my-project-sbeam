      ...
      <main class="game-list">
        ...
        >
          ...
          <div class="info">
            ...
            <p class="price">价格：￥{{ game.gameOriginalPrice }}</p>
          </div>
        </div>
      </main>
    </div>
...
onMounted(async () => {
  try {
    const res = await axios.get('http://localhost:8080/sales/games') // ← 改：访问后端接口
    gameList.value = res.data.data || [] // 后端返回统一结构 { code, msg, data }
  } catch (err) {
...
// 搜索 + 分类 + 排序
const filteredGames = computed(() => {
  let result = gameList.value.filter((g) => g.gameName.includes(keyword.value))
  if (activeCategory.value !== '全部') {
    result = result.filter((g) => g.category === activeCategory.value)
  }
  if (sortType.value === 'price') {
    result.sort((a, b) => a.gameOriginalPrice - b.gameOriginalPrice)
