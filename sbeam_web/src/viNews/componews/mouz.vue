...
window.addEventListener("mousemove", function (evt) { //监听鼠标移动事件
  for (let  i = 0; i < 15; i++) { //添加15个粒子
    points.push({
      ...
      vy: 0.5 - Math.random(),
      life: live, //存活周期
      color: colors[parseInt((Math.random() * colors.length).toString())], //随机选择颜色
...
window.addEventListener("click", function (evt) { //监听点击事件
  for (let  i = 0; i < 100; i++) {
    clicks.push({
      sx: evt.x,
      sy: evt.y,
      color: colors[parseInt((Math.random() * colors.length).toString())],
      life: live,
      vx: 0.5 - Math.random(), //x轴及y轴的移动向量，取值范围为-0.5 ~ 0.5
      vy: 0.5 - Math.random(),
    })
...
function drawpoints() { //绘制粒子
  ...
  for (let  i = 0; i < points.length; i++) { //遍历粒子
    ...
    ctx.beginPath()
    ctx.arc(point.sx, point.sy, point.size, Math.PI * 2, 0) //根据粒子属性画圆
